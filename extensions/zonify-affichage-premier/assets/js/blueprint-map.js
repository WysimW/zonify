...
(function($) {
    ...
    const BlueprintMap = {
        ...
        setupMapControls: function() {
            ...
            $(document).on('zonify_map_ready', function(event, map) {
                ...
                centerButton.on('click', function() {
                    ...
                    const defaultCenter = window.zonifyMapData && window.zonifyMapData.defaultCenter 
                        ? window.zonifyMapData.defaultCenter 
                        : [46.227638, 2.213749]; // Centre de la France par défaut
                    
                    const defaultZoom = window.zonifyMapData && window.zonifyMapData.defaultZoom
                        ? window.zonifyMapData.defaultZoom
                        : 6;
        ...
        enhancePopups: function() {
            $(document).on('zonify_popup_opened', function(event, popup) {
        ...
        setupQuickFilters: function() {
            ...
            if (window.zonifyMapData && window.zonifyMapData.panelTypes) {
                const types = window.zonifyMapData.panelTypes;
                ...
                $('.filter-btn').on('click', function() {
                    ...
                    $(document).trigger('zonify_filter_markers', { type: type });
                });
                
                // Hook l'événement de filtre
                $(document).on('zonify_filter_markers', function(event, data) {
                    // Cette fonction sera implémentée par Zonify
        ...
        createMapLegend: function() {
            ...
            if (window.zonifyMapData && window.zonifyMapData.panelTypes) {
                ...
                $.each(window.zonifyMapData.panelTypes, function(index, type) {
                ...
                $('<style>', {
                    text: `
                        ...
                        .quick-filters h3::after {
                            ...
                            width: 40px;
                            height: 3px;
                            background: var(--zonify-secondary);
                            border-radius: 3px;
                        }
                        ...
                        .filter-btn:hover {
                            border-color: var(--zonify-primary);
                            color: var(--zonify-primary);
                        }
                        
                        .filter-btn.active {
                            background: var(--zonify-primary);
                            color: white;
                            border-color: var(--zonify-primary);
                        }

 ...
 */
function zonify_register_icon_post_type() {
    $labels = array(
        'name'               => 'Icônes POI',
    ...
    $args = array(
        ...
        'show_ui'            => true,
        'show_in_menu'       => false, // Sera intégré dans le menu Zonify
    ...
    register_post_type('zonify_icon', $args);
}
add_action('init', 'zonify_register_icon_post_type');

/**
 * Ajouter une méta box pour les paramètres de l'icône
 */
function zonify_icon_add_meta_box() {
    add_meta_box(
        'zonify_icon_settings',
        'Paramètres de l\'icône',
        'zonify_icon_settings_callback',
        'zonify_icon',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'zonify_icon_add_meta_box');

/**
 * Callback pour la méta box des paramètres d'icône
 */
function zonify_icon_settings_callback($post) {
    ...
    wp_nonce_field('zonify_icon_settings_nonce', 'zonify_icon_settings_nonce');
    
    // Afficher les champs
    ?>
    <p>
        <strong><?php _e('Note:', 'zonify'); ?></strong> 
        <?php _e('Veuillez définir une image mise en avant pour cette icône en utilisant le panneau "Image mise en avant".', 'zonify'); ?>
    </p>
    
    <table class="form-table">
        <tr>
            <th><label for="icon_width"><?php _e('Largeur (px)', 'zonify'); ?></label></th>
            <td>
                <input type="number" id="icon_width" name="icon_width" value="<?php echo esc_attr($icon_width); ?>" min="1" />
            </td>
        </tr>
        <tr>
            <th><label for="icon_height"><?php _e('Hauteur (px)', 'zonify'); ?></label></th>
            <td>
                <input type="number" id="icon_height" name="icon_height" value="<?php echo esc_attr($icon_height); ?>" min="1" />
            </td>
        </tr>
        <tr>
            <th><label><?php _e('Point d\'ancrage', 'zonify'); ?></label></th>
            <td>
                <p><?php _e('Point d\'ancrage de l\'icône (en pixels depuis le coin supérieur gauche):', 'zonify'); ?></p>
                ...
                <p class="description"><?php _e('Généralement, X = largeur/2 et Y = hauteur (pour que la pointe de l\'icône soit sur le point exact).', 'zonify'); ?></p>
            </td>
        </tr>
    </table>
    <?php
 ...
 */
function zonify_save_icon_settings($post_id) {
    // Vérifier le nonce
    if (!isset($_POST['zonify_icon_settings_nonce']) || !wp_verify_nonce($_POST['zonify_icon_settings_nonce'], 'zonify_icon_settings_nonce')) {
        return;
    }
...
add_action('save_post_zonify_icon', 'zonify_save_icon_settings');

/**
 * Ajouter la colonne d'aperçu dans la liste des icônes
 */
function zonify_icon_columns($columns) {
    $new_columns = array();
...
add_filter('manage_zonify_icon_posts_columns', 'zonify_icon_columns');

/**
 * Afficher l'aperçu de l'icône dans la colonne
 */
function zonify_icon_column_content($column, $post_id) {
    if ($column === 'preview') {
...
add_action('manage_zonify_icon_posts_custom_column', 'zonify_icon_column_content', 10, 2);